...
//
// This delegates to pq.StringArray for encoding/decoding rather than a
// hand-rolled parser: a naive split-on-comma implementation corrupts any
// element containing a comma, brace, backslash, or embedded quote (e.g. an
// API key scope like `"read,write"` or a path pattern containing a comma),
// silently truncating or misplacing values on every round trip.
...
func (a *StringArray) Scan(value interface{}) error {
	...
	if err := pqArray.Scan(value); err != nil {
		return fmt.Errorf("failed to unmarshal StringArray value: %w", err)
	}
